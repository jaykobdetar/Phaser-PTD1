...
export function createMoveRuntime(battle,options={}) {
  ...
  const C={Sprite:display.Sprite},S={},E={int,uint,Number,String,Boolean,Array,Object,NaN,Infinity,undefined,Math:Object.assign(Object.create(Math),{random:rng}),superBase(){},as(value,Type){return value==null?null:value instanceof Type?value:null;},trace(...args){events.push({kind:'trace',args});},sortOn(array,key,flags){return array.sort((a,b)=>((flags&16)?Number(a[key])-Number(b[key]):String(a[key]).localeCompare(String(b[key])))*((flags&2)?-1:1));},class_1:{get var_359(){return battle.save?.settings?.damageText===false?0:1;},var_194:1}};
  ...
  function actor(f){
    ...
    a.take_Damage=(amount,effect,multiplier=0,sound=true)=>{if(disposed||a._fainting)return;const before=a.life;originalDamage.call(a,amount,effect,multiplier,sound);events.push({kind:'damage',source:currentSource?.fighter,target:f,amount:before-a.life,multiplier,sourceClass:currentMove?.sourceClass});emit('damage',{source:currentSource?.fighter,target:f,amount:before-a.life,multiplier,native:true});};
    a.take_Healing=amount=>{if(disposed)return;const before=a.life;originalHeal.call(a,amount);events.push({kind:'heal',target:f,amount:a.life-before});emit('heal',{target:f,amount:a.life-before,native:true});};
